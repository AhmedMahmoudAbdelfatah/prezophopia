...
export default function Post(props) {
    ...
    const likeAction = async (e) => {
        ...
        if (e.target.classList.contains("liked"))  e.target.classList.remove("liked");
        else e.target.classList.add("liked");
        try {
            const response = await axios.post(`http://localhost:8080/api/post/like/${props.id}/1`, {  }, {
                headers: {
        ...
        <div class="post" key={props.id}>
            ...
            <footer>
                ...
                <div className="actions">
                    <div role={"button"} className={`feed ${props.myFeed > 0? "liked" : ""}`} onClick={likeAction}> <FontAwesomeIcon icon={faThumbsUp} style={{height:"16px", marginRight: "5px"}} /> like</div>
